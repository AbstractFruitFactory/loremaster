<script lang="ts">
	...
	const sortedRevisions = $derived(history.current ? [...history.current].reverse() : [])
	...
	async function restoreRevision(request: { revisionId: string; currentRevisionId: string }) {
