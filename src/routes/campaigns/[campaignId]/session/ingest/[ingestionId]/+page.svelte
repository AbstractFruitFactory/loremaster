<script lang="ts">
	...
	const chronologyEndpointSelected = (proposalId: string) => {
		const proposal = eventProposalById().get(proposalId)
		return proposal ? selected(proposal) : false
	}

	const eventTitleFor = (proposalId: string) =>
		eventProposalById().get(proposalId)?.title ?? 'Unknown event'

	const selectedChronology = () =>
		(draft.current?.chronology ?? []).filter(
			(relation) =>
				chronologyStatus(relation) === 'approved' &&
				chronologyEndpointSelected(relation.beforeProposalId) &&
				chronologyEndpointSelected(relation.afterProposalId)
		)
...
{#snippet chronologyCard(relation: SessionChronologyProposal)}
	...
	>
		<div class="chronology-relation">
			<span>{eventTitleFor(relation.beforeProposalId)}</span>
			<Icon icon="lucide:arrow-right" aria-label="happened before" />
			<span>{eventTitleFor(relation.afterProposalId)}</span>
		</div>
		<p>{relation.reason}</p>
		<div class="proposal-footer">
			{#if relation.certainty === 'inferred'}
				<span class="badge attention">Inferred order</span>
			{/if}
			<div class="decision-actions">
				<button
					type="button"
