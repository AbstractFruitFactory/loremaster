...
const canonicalSubjectExtractionGuidance =
	"Assign every entity reference a role. Subject means the stable fact belongs in that entity's canon; related means the entity is materially mentioned for linking or context but must not receive the complete fact. Every stable-fact claim must have one clear subject. Split compound statements into separate atomic stable facts when different details belong to different subjects, even when those details occur simultaneously. Do not copy a broad multi-entity sentence onto each subject. For example, three relics reacting to one bell should yield one focused stable fact for each relic with that relic as subject, plus a development for their simultaneous shared reaction with all three relics as related references. Details belonging specifically to a war, location, person, or item must be separate from details belonging to another subject. A named durable event or period may be the subject of a stable fact and may exist without any known before, after, or during relationship. Do not require chronology before identifying it. If a source independently establishes both an entity fact and the identity of a named event or period, emit separate atomic claims for each subject. For example, 'Dereka served during the War of the Ages' supports a fact about Dereka and independently identifies the War of the Ages as a named period. For every event reference, set eventForm to period for an extended span such as a war, reign, age, journey, or festival, occurrence for a discrete incident, and null only when the evidence does not establish which form applies. Set eventForm to null for non-event references. For development claims, the eventTitle identifies a discrete occurrence and participating entities are related references. For mention claims, mark the primarily mentioned entity as subject and contextual entities as related."

const canonicalSubjectValidationGuidance =
	'Validate each supplied entity-reference role as part of reference validation. For a stable fact, accept subject only when the exact claim is canonically about that entity, and accept related only when the entity is materially referenced without owning the complete fact. Reject a subject role that would copy a compound or contextual claim onto the wrong document. A supported stable fact should have one accepted subject. A named durable event or period is valid even when its relative chronology is unknown. For an event reference, also reject the reference when its eventForm conflicts with the evidence; period means an extended temporal span and occurrence means a discrete incident. For a development, participant references must be related because the event title, not a participant document, owns the development. For a mention, the primarily mentioned entity may be subject. Role mistakes affect the reference decision, not whether independently supported claim content is true.'

export const extractionSystem =
	canonicalSubjectExtractionGuidance +
	' ' +
...
export const validationSystem =
	canonicalSubjectValidationGuidance +
	' ' +
