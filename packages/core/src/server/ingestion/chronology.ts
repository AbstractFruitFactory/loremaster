...
export const validateChronology = (
	...
	const proposalEvents = new Map(eventProposals.map((proposal) => [proposal.proposalId, proposal]))
	...
	for (const relation of relations) {
		...
		if (
			...
			warnings.push(`Chronology relation discarded [invalid-reference] ${JSON.stringify(relation)}`)
			continue
		}
	...
	for (const decision of inference.coverage) {
		if (!proposalEvents.has(decision.eventId) || coverageByEventId.has(decision.eventId)) {
			warnings.push(`Chronology coverage discarded [invalid-reference] ${JSON.stringify(decision)}`)
			continue
		}
	...
	const coverage: SessionChronologyCoverageProposal[] = eventProposals.map((proposal) => {
		...
		if (!decision) {
			warnings.push(`Chronology coverage missing for event ${JSON.stringify(proposal.proposalId)}`)
			return {
				event,
				status: 'missing',
...
export const chronology = (
	...
	const inferChronology = (
		...
		return pipe(
			...
			map((inference) => validateChronology(inference, eventProposals, existingEvents))
		)
	}
