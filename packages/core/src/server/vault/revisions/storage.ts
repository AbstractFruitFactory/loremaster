...
import type { VaultRevision } from './types.js'

type RevisionStorageFailure<Operation extends string> = Failure<'revisionStorage', Operation>

export type RevisionStorage = {
	write: (revision: VaultRevision) => Effect<VaultRevision, RevisionStorageFailure<'writeRevision'>>
	getRevision: (
		campaignId: string,
		documentId: string,
		revisionId: string
			...
			tryPromise({
				try: async () => {
					...
					try {
						...
						const existing = await readJson<VaultRevision>(path)
						if (
