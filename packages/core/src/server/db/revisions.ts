...
import type { RevisionHead, VaultRevision } from '../vault/revisions/types.js'
import { db } from './index.js'
import { vaultRevisionHeads, vaultRevisions } from './schema.js'

export { withDatabaseAdvisoryLocks as withAdvisoryLocks } from './index.js'
		...
		try: () =>
			db.transaction(async (transaction) => {
				await transaction
					...
					.onConflictDoNothing()
				const whereDocument = and(
					eq(vaultRevisionHeads.campaignId, revision.campaignId),
					eq(vaultRevisionHeads.documentId, revision.documentId)
				)
				...
				if (!existing) {
					await transaction
						...
						.onConflictDoNothing()
				} else if (existing.revisionId !== revision.revisionId) {
					if (existing.revisionId !== revision.previousRevisionId) {
						throw Error('Revision head advanced before deterministic revision indexing')
					}
					const updated = await transaction
						.update(vaultRevisionHeads)
						.set(headValues(revision))
						.where(
							and(whereDocument, eq(vaultRevisionHeads.revisionId, revision.previousRevisionId))
						)
						.returning({ revisionId: vaultRevisionHeads.revisionId })
					if (!updated.length) {
						throw Error('Revision head advanced during deterministic revision indexing')
					}
				}
				const [indexedHead] = await transaction
					.select()
					.from(vaultRevisionHeads)
					.where(whereDocument)
					.limit(1)
				if (
					indexedHead?.revisionId !== revision.revisionId ||
					indexedHead.path !== revision.path ||
					indexedHead.sourceHash !== revision.afterHash
				) {
					throw Error('Revision head does not match indexed revision')
				}
			}),
